<script lang="ts">
  ...
  const chartConfigs: Record<
    string,
    Record<string, { address: string; name: string }>
  > = {
    flare: {
      cysFLR: {
  ...
  function updateChart() {
    ...
    const networkForToken =
      supportedNetworks.find((n) => n.chain.id === $selectedCyToken.chainId) ||
      $selectedNetwork;
    ...
    if (walletConfig) {
      switchNetwork(walletConfig, { chainId: networkForToken.chain.id }).catch(
        (error) =>
          console.warn(
            `Failed to switch wallet network to ${networkForToken.chain.id}:`,
            error,
          ),
      );
    }
    ...
    const chartConfig = networkConfigs[
      $selectedCyToken.symbol as keyof typeof networkConfigs
    ] || {
...
<div
  class="flex w-full flex-col items-center justify-center gap-4 p-4 sm:gap-6 sm:p-6"
>
  <div
