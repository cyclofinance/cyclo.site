...
<div class="flex flex-grow flex-col items-center gap-6 bg-[#1C02B8] p-2 sm:p-6">
	<div class="flex h-fit w-full max-w-5xl gap-6" data-testid="action-tabs">
		<HrefButton href={base + '/lock'} class="flex-1" inset={$page.url.pathname === '/lock'}
			>LOCK</HrefButton
		>
		<HrefButton href={base + '/unlock'} class="flex-1" inset={$page.url.pathname === '/unlock'}
			>UNLOCK</HrefButton
		>
	</div>
	<slot />
