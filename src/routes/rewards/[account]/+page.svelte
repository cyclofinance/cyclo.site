      ...
      >
        ...
        <div class="flex flex-col gap-2 text-center">
          ...
          <p class="text-gray-300">
            Switching to Flare network. Please approve the network switch in
            your wallet.
          </p>
          <p class="text-sm text-gray-400">
            Current network: <span class="font-semibold"
              >{$selectedNetwork.chain.name}</span
            >
          </p>
        </div>
      </div>
    ...
    {#key networkKey}
      ...
      {:else}
        <Card customClass="items-stretch">
          <div
            class="space-y-6"
            data-testid="account-status-network-unavailable"
          >
            <div class="flex flex-col gap-2">
              ...
              <p class="text-gray-300">
                Account rewards status is currently only available on Flare
                network. Please switch to Flare to view account information.
              </p>
              <p class="text-sm text-gray-400">
                Current network: <span class="font-semibold"
                  >{$selectedNetwork.chain.name}</span
