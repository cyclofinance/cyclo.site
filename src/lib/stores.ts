...
const networkConfigMap = new Map(
  supportedNetworks.map((config) => [config.key, config]),
);
...
export const activeNetworkConfig = derived(activeNetworkKey, (key) =>
  resolveNetworkConfig(key),
);
...
export const targetNetwork = derived(
  activeNetworkConfig,
  (config) => config.chain,
);
