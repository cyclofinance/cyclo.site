...
describe("prices/index", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(DataFetcher).mockImplementation(
    ...
    vi.mocked(Router.findBestRoute).mockReturnValue(
      mockRoute as unknown as MultiRoute,
    );
  ...
  describe("getPrice", () => {
    ...
    it("should format the output amount correctly", async () => {
      ...
      expect(formatUnits).toHaveBeenCalledWith(
        mockRoute.amountOutBI,
        mockOutputToken.decimals,
      );
    });
