...
export const getPrice = async (
  ...
  try {
    ...
    const outputTokenChainId = isCyToken(outputToken)
      ? outputToken.chainId
      : undefined;
    const inputTokenChainId = isCyToken(inputToken)
      ? inputToken.chainId
      : undefined;
    ...
    const chainId =
      tokenChainId ?? selectedNetworkChainId ?? dataFetcherChainId ?? flare.id;
    ...
    if (chainId === arbitrum.id) {
      ...
      const network = supportedNetworks.find(
        (n: NetworkConfig) => n.chain.id === arbitrum.id,
      );
      if (!network) {
      ...
      const client = getPublicClient(wagmiConfigValue, {
        chainId: arbitrum.id,
      });
      if (!client) {
