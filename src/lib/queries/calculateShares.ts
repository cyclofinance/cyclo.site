...
import {
  aggregateTotalEligibleFromVaults,
  extractBalancesFromVaults,
} from "./vaultUtils";
...
export const calculateShares = (
  ...
  const rewardsPools = calculateRewardsPools(
    eligibleTotalsForPools,
    currentTokens,
  );
  ...
  for (const token of currentTokens) {
    ...
    shares[token.symbol].rewardsAmount =
      (shares[token.symbol].percentageShare * poolReward) / ONE;
