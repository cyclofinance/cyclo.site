...
const { mockActiveNetworkKey, mockSupportedNetworks, mockSwitchNetwork } =
  vi.hoisted(() => {
...
vi.mock("$lib/stores", async () => {
  ...
  return {
    ...
    setActiveNetwork: (key: string) => {
      if (mockSupportedNetworks.some((n) => n.key === key))
        mockActiveNetworkKey.set(key);
    },
  };
});
...
describe("NetworkSelector", () => {
  ...
  it("trims option labels (DOM whitespace safe)", () => {
    ...
    expect(options[0].textContent?.trim()).toBe(
      mockSupportedNetworks[0].chain.name,
    );
    expect(options[1].textContent?.trim()).toBe(
      mockSupportedNetworks[1].chain.name,
    );
  });
  ...
  it("calls switchNetwork with the right arguments exactly once per change", async () => {
    ...
    expect(mockSwitchNetwork).toHaveBeenCalledWith(mockWeb3Config, {
      chainId: 999,
    });
  });
