...
describe("DsfStrategy Component", () => {
  ...
  it("should pass form values to handleDeployDsf", async () => {
    ...
    const callArgs = vi.mocked(transactionStore.handleDeployDsf).mock
      .calls[0][0];
  ...
  it("should show custom deposit amount inputs when toggled", async () => {
    ...
    expect(
      screen.queryByTestId("deposit-amount-input"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("rotate-deposit-amount-input"),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.getByTestId("rotate-deposit-amount-input"),
    ).toBeInTheDocument();
  });
  ...
  it("should update advanced option values", async () => {
    ...
    const nextTradeMultiplierInput = screen.getByTestId(
      "next-trade-multiplier-input",
    );
    await fireEvent.input(nextTradeMultiplierInput, {
      target: { value: "1.05" },
    });

    // Update Cost Basis Multiplier
    const costBasisMultiplierInput = screen.getByTestId(
      "cost-basis-multiplier-input",
    );
    await fireEvent.input(costBasisMultiplierInput, {
      target: { value: "1.02" },
    });
