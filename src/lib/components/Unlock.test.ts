...
describe("Unlock Component", () => {
  ...
  it("should show connect wallet button when wallet is not connected", () => {
    ...
    expect(
      screen.getByText("CONNECT WALLET TO VIEW RECEIPTS"),
    ).toBeInTheDocument();
  });
  ...
  it("should display receipts table when receipts are available", async () => {
    ...
    const { refreshReceiptsForToken } = await import(
      "$lib/queries/refreshReceiptsForToken"
    );
    vi.mocked(refreshReceiptsForToken).mockImplementation(
      async (_signerAddress, _tokenName, setLoading) => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("NO cysFLR RECEIPTS FOUND..."),
  ...
  it('should show "NO RECEIPTS FOUND" message when no receipts are available', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("NO cysFLR RECEIPTS FOUND..."),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should display correct token name when a different token is selected", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("NO cyWETH RECEIPTS FOUND..."),
      ).toBeInTheDocument();
    });
  });
});
