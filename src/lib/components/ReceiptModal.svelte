...
>
  <div
    class="flex w-full flex-col justify-between font-semibold text-white sm:flex-row"
  >
    ...
    <div class="flex flex-col">
      <Input
        ...
        on:input={(event) => {
          ...
          try {
            amountToRedeem = parseUnits(
              readableAmountToRedeem.toString(),
              token.decimals,
            );
  ...
  <div
    class="hidden w-full flex-col items-center justify-center font-semibold text-white sm:flex"
  >
    <div class="flex w-full flex-row justify-center gap-12 text-right">
      ...
      >
      <span class="w-1/2 text-center"
        >{!readableAmountToRedeem ? 0 : readableAmountToRedeem}
        {receipt.token}</span
      >
    </div>
  ...
  >
    <div
      class="flex w-full flex-col items-center justify-center gap-1 text-right"
    >
      <span class="w-full text-center"
        >{readableAmountToRedeem === null ? 0 : readableAmountToRedeem}
        {receipt.token}</span
      >
      ...
      <div class="flex flex-row items-center gap-2 overflow-ellipsis">
        <span
          ...
          data-testid="flr-to-receive-mobile"
        >
          {formatUnits(sFlrToReceive, token.decimals)}
          {token.underlyingSymbol}
        </span>
