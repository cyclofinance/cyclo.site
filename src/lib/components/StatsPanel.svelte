...
<Card>
  ...
  {:else if stats}
    <div
      class="grid w-full grid-cols-1 gap-8 md:grid-cols-4"
    ...
    >
        ...
        <div class="space-y-2">
          {#each $tokens as token}
            {@const apyValue = Number(
              formatEther(stats.apy[token.symbol] || 0n),
            )}
            {@const apyDisplay =
              apyValue === 0
                ? "0.0000"
      ...
      <div class="space-y-4">
        ...
        <div class="font-mono text-3xl font-bold text-white">
          {Number(formatEther(stats.totalEligibleSum))
            .toFixed(2)
            .toLocaleString()}
        </div>
        <div class="space-y-1 font-mono text-sm text-gray-400">
          {#each $tokens as token}
            <div>
              {token.symbol}: {Number(
                formatUnits(
                  stats.totalEligible[token.symbol] || 0n,
                  token.decimals,
                ),
              ).toFixed(2)}
            </div>
