...
{#if loading}
  ...
  >
    <div
      class="h-8 w-8 animate-spin rounded-full border-b-2 border-t-2 border-white"
    />
  </div>
{:else if error}
        ...
        <div class="space-y-2">
          {#each [...stats.transfers.in, ...stats.transfers.out, ...stats.liquidityChanges]
            ...
            >
                  ...
                  {#if "fromIsApprovedSource" in transfer}
                    {#if transfer.from.id.toLowerCase() === account.toLowerCase()}
                      <span class="text-error"
                        >Sent to {transfer.to.id.slice(
                          0,
                          6,
                        )}...{transfer.to.id.slice(-4)}</span
                      >
                    {:else}
                      <span class="text-success"
                        >Received from {transfer.from.id.slice(
                          0,
                          6,
                        )}...{transfer.from.id.slice(-4)}</span
                      >
                    {/if}
                  {:else}
                    <span class="text-error"
              ...
              <div
                class="flex items-center gap-2 truncate pl-4 font-mono text-white"
              >
                <span class="text-xs text-gray-300"
                  >{currentTokens.find((t) =>
                    isAddressEqual(transfer.tokenAddress, t.address),
                  )?.symbol || "Unknown"}</span
                >
                <span
                  >{formatUnits(
