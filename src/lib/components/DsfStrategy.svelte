<script lang="ts">
  ...
  $: selectedNetworkForRotateToken = rotateToken
    ? supportedNetworks.find(
        (network) => network.chain.id === rotateToken!.chainId,
      ) || supportedNetworks[0]
    : supportedNetworks[0];

  // Keep the app's active network aligned with the rotateToken's network
  $: if (
    selectedNetworkForRotateToken?.key &&
    selectedNetworkForRotateToken.key !== lastSyncedNetworkKey
  ) {
...
<div class="grid w-full grid-cols-1 items-start gap-6">
  <div>
    <h1 class="text-2xl text-white">
      Deploy a Dynamic Spread Fast (DSF) strategy
    </h1>
    <p class="mt-2 text-sm text-gray-200">
      A market-making order that offers two-sided auction-based spreads that
      narrow over time, with a defensive feature that quickly exits positions by
      counter-trading when market trends emerge.
    </p>
    <p class="mt-2 text-sm text-gray-200">
      This strategy is powered by Raindex. Once you've deployed you can manage
      your order and withdraw the funds on <a
        target="_blank"
        class="underline"
        href="https://v2.raindex.finance">the Raindex platform</a
      >.
    </p>
  </div>
  <div class="flex flex-col gap-2">
    <!-- Amount Token -->
    <span class="text-sm text-gray-200">Amount Token</span>
    ...
    <!-- Rotate Token -->
    <span class="text-sm text-gray-200">Rotate Token</span>
    <!-- Rotate Token -->
  ...
  <div class="flex flex-col gap-2">
    <div class="flex items-center gap-2">
      ...
      <span class="text-sm text-gray-400"
        >{amountTokenFastExit ? "On" : "Off"}</span
      >
    </div>
    <div class="flex items-center gap-2">
      ...
      <span class="text-sm text-gray-400"
        >{rotateTokenFastExit ? "On" : "Off"}</span
      >
    </div>
  </div>
  ...
  <div class="flex flex-col gap-2" data-testid="baseline-container">
    {#if amountToken && rotateToken}
      <div class="text-sm text-gray-200">
        ...
        <InfoTooltip
          >The initial price of the {amountToken.symbol} in the {rotateToken.symbol}
          market.
        </InfoTooltip>
      </div>
    {/if}
  ...
  <div class="flex flex-col gap-2" data-testid="amount-container">
    <span class="text-sm text-gray-200"
      ...
      <InfoTooltip
        >The maximum amount of {amountToken?.symbol} that will be offered in a single
        auction.
      </InfoTooltip></span
    >
    {#if amountToken}
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-2">
          <span class="text-sm text-gray-200">
            Next Trade Multiplier
            <InfoTooltip
              >This is the most the order will move the price in a single trade.
              Larger numbers will capture larger price jumps but trade less
              often, smaller numbers will trade more often but be less defensive
              against large price jumps in the market.
            </InfoTooltip>
          </span>
          <Input
            type="number"
        ...
        <div class="flex flex-col gap-2">
          <span class="text-sm text-gray-200">
            Cost Basis Multiplier
            <InfoTooltip
              >The minimum spread applied to the breakeven in addition to the
              auction. This is applied in both directions so 1.01x would be a 2%
              total spread.
            </InfoTooltip>
          </span>
          <Input
        ...
        <div class="flex flex-col gap-2">
          <span class="text-sm text-gray-200">
            Time Per Epoch
            <InfoTooltip
              >The amount of time (in seconds) between halvings of the price and
              the amount offered during each auction, relative to their
              baselines.
            </InfoTooltip>
          </span>
          <Input
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex flex-col gap-2">
            <span class="text-sm text-gray-200"
              >{amountToken?.symbol} Input Vault ID</span
            >
            <VaultIdInput
              bind:vaultId={amountTokenInputVaultId}
              bind:isError={amountTokenInputVaultIdError}
            />
          </div>
          <div class="flex flex-col gap-2">
            <span class="text-sm text-gray-200"
              >{rotateToken?.symbol} Input Vault ID</span
            >
            <VaultIdInput
              bind:vaultId={rotateTokenInputVaultId}
              bind:isError={rotateTokenInputVaultIdError}
            />
          </div>
          <div class="flex flex-col gap-2">
            <span class="text-sm text-gray-200"
              >{amountToken?.symbol} Output Vault ID</span
            >
            <VaultIdInput
              bind:vaultId={amountTokenOutputVaultId}
              bind:isError={amountTokenOutputVaultIdError}
            />
          </div>
          <div class="flex flex-col gap-2">
            <span class="text-sm text-gray-200"
              >{rotateToken?.symbol} Output Vault ID</span
            >
            <VaultIdInput
              bind:vaultId={rotateTokenOutputVaultId}
              bind:isError={rotateTokenOutputVaultIdError}
            />
          </div>
        </div>
      </div>
  ...
  <Button
    ...
    disabled={disableDeploy}>Deploy</Button
  >
</div>
