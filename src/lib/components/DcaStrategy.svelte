<script lang="ts">
  ...
  $: if ($allTokens.length > 0) {
    ...
    const storeTokenValid = currentTokens.some(
      (t) => t.name === storeToken.name,
    );
  ...
  $: selectedNetworkForCyToken =
    supportedNetworks.find(
      (network) => network.chain.id === selectedCyToken?.chainId,
    ) || supportedNetworks[0];

  // Keep the app's active network aligned with the selected cyToken's network
  $: if (
    selectedNetworkForCyToken?.key &&
    selectedNetworkForCyToken.key !== lastSyncedNetworkKey
  ) {
  ...
  $: selectedAmountToken =
    selectedBuyOrSell == "Buy"
      ? selectedToken ||
        (networkTokens.length > 0 ? networkTokens[0] : undefined)
      : selectedCyToken;
  $: depositAmount = chooseOverrideDepositAmount
...
<div class="grid w-full grid-cols-1 items-start gap-6">
  <div>
    ...
    <p class="mt-2 text-sm text-gray-200">
      This is a streaming DCA strategy. You can deploy it to spread out buys or
      sells of cyTokens over time. Very useful when the slippage is too high for
  ...
  <div class="flex flex-col gap-2" data-testid="amount-container">
    <span class="text-sm text-gray-200"
      ...
      <InfoTooltip
        >The number of tokens you will {selectedBuyOrSell == "Buy"
          ? "spend"
          : "sell"} every period of time defined below. This creates a streaming
        budget over time.
      </InfoTooltip></span
    >
  ...
  <div class="flex flex-col gap-2" data-testid="period-container">
    <span class="text-sm text-gray-200"
      >Every
      <InfoTooltip
        >The strategy will attempt to {selectedBuyOrSell == "Buy"
          ? "spend"
          : "sell"}
    ...
    {#if selectedToken}
      <div class="text-sm text-gray-200">
        {selectedBuyOrSell == "Buy"
          ...
          : "Lowest price to sell"} at ({`${selectedToken.symbol} per ${selectedCyToken.symbol}`})
        <InfoTooltip
          >The {selectedBuyOrSell == "Buy" ? "highest" : "lowest"} price at which
          the strategy will {selectedBuyOrSell == "Buy" ? "buy" : "sell"}. If
          the market price won't allow it, the strategy will not buy or sell,
          but note the budget set above is still accruing, meaning the strategy
          will attempt to "catch up" when the market comes back within range.
        </InfoTooltip>
      </div>
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex flex-col gap-2">
            ...
            <VaultIdInput
              bind:vaultId={inputVaultId}
              bind:isError={inputVaultIdError}
            />
          </div>
          <div class="flex flex-col gap-2">
            <span class="text-sm text-gray-200">Output vault id</span>
            <VaultIdInput
              bind:vaultId={outputVaultId}
              bind:isError={outputVaultIdError}
            />
          </div>
        </div>
      </div>
