...
describe("DcaStrategy Component", () => {
  ...
  const fillRequiredFields = async () => {
    await fireEvent.input(screen.getByTestId("amount-input"), {
      target: { value: "100" },
    });
    await fireEvent.input(screen.getByTestId("period-input"), {
      target: { value: "7" },
    });
    await fireEvent.input(screen.getByTestId("baseline-input"), {
      target: { value: "1.5" },
    });
  };
  ...
  it("should update selectedAmountToken when Buy/Sell selection changes", async () => {
    ...
    const firstCallArgs = vi.mocked(transactionStore.handleDeployDca).mock
      .calls[0];
  ...
  it("should handle vault ID inputs correctly", async () => {
    ...
    const inputVaultIdInput = vaultIdInputs.find(
      (input) =>
        input.closest("div")?.previousElementSibling?.textContent ===
        "Input vault id",
    );
  ...
  it("should show error for invalid vault IDs", async () => {
    ...
    expect(
      screen.getByText("Invalid vault id: must be a valid hex string"),
    ).toBeInTheDocument();
