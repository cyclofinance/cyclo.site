...
if (import.meta.vitest) {
  ...
  it("should return the slug from a path", () => {
    ...
    expect(slugFromPath("src/docs/20-some-article.svelte.md")).toEqual(
      "some-article",
    );
    expect(slugFromPath("src/docs/30-some-article.svx")).toEqual(
      "some-article",
    );

    expect(slugFromPath("src/docs/category/40-some.svx")).toEqual("some");
    expect(slugFromPath("src/docs/category/50-some-article.svx")).toEqual(
      "some-article",
    );
  });
