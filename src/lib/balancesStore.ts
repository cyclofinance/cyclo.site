...
const getDepositPreviewSwapValue = async (
  ...
  try {
    ...
    try {
      const { result: swapQuote } = await simulateQuoterQuoteExactInputSingle(
        config,
        {
          address: get(quoterAddress),
          args: [
...
const getLockPrice = async (
  config: Config,
  selectedToken: CyToken,
  chainId?: number,
) => {
  const { result } = await simulateErc20PriceOracleReceiptVaultPreviewDeposit(
    config,
    {
      address: selectedToken.address,
      args: [BigInt(1e18), 0n],
...
const balancesStore = () => {
  ...
  const refreshPrices = async (config: Config, selectedToken: CyToken) => {
    ...
    const [lockPrice, cysFlrSupply, underlyingBalanceLockedInCysToken] =
      await Promise.all([
        getLockPrice(config, selectedToken).catch((error) => {
          console.log(
            `Failed to fetch lock price for ${selectedToken.name}:`,
            error,
          );
          return BigInt(0);
        }),
        getcysFLRSupply(config, selectedToken).catch((error) => {
          console.log(
            `Failed to fetch supply for ${selectedToken.name}:`,
            error,
          );
          return BigInt(0);
        }),
  ...
  const refreshFooterStats = async (config: Config) => {
    const getTokenStats = async (token: CyToken, network: NetworkConfig) => {
      const [supplyResult, priceResult, lockPriceResult, tvlResult] =
        await Promise.all([
          ...
          getLockPriceFooterStats(config, token, token.chainId).catch(
            (error) => {
              console.log(
                `Failed to fetch lock price for ${token.name}:`,
                error,
              );
              return BigInt(0);
            },
      ...
      const stats = {
        ...
        usdTvl:
          lockPriceResult > 0n
            ? (tvlResult * lockPriceResult) / BigInt(1e18)
            : BigInt(0),
      };
