...
const transactionStore = () => {
  ...
  const handleLockTransaction = async ({
    ...
    const writeLock = async () => {
      ...
      try {
        ...
        const getReceiptsResult = await refreshAllReceipts(
        ...
        return transactionError(
          TransactionErrorMessage.BALANCE_REFRESH_FAILED,
          hash,
        );
      }
      // SUCCESS
  ...
  const deployStrategy = async (
    ...
    for (const approval of deploymentArgs.approvals) {
      awaitWalletConfirmation(
        `Awaiting wallet confirmation to approve ${approval.symbol}...`,
      );
    ...
    const interval = setInterval(async () => {
      ...
      if (
        currentNetwork.key !== initialNetworkKey ||
        currentNetwork.chain.id !== initialChainId
      ) {
  ...
  const handlePythPriceUpdate = async () => {
    ...
    awaitWalletConfirmation(
      `Awaiting wallet confirmation to update oracle price..`,
    );
    // 5) Submit
    let hash: Hex | undefined;
    try {
